...
## Шаг 2. Подготовка каталога и ключа шифрования
...
2. **Ключ шифрования.** Сгенерируйте его сами и сохраните в надёжном месте — по нему расшифровываются все креды в бэкапах:
   ```bash
   openssl rand -hex 32
   ```
   Если не задать `N8N_ENCRYPTION_KEY`, n8n сгенерирует ключ сам и положит в `/opt/n8n-data/config`. Потеряли этот файл — потеряли доступ ко всем сохранённым учётным данным, восстановить их нельзя.

3. **Файл с переменными окружения.** Создайте `/opt/n8n.env`:
   ```bash
   sudo nano /opt/n8n.env
   ```

   Содержимое (подставьте свой домен, часовой пояс и ключ из предыдущего пункта):
   ```ini
   ...
   TZ=Europe/Moscow
   N8N_ENCRYPTION_KEY=вставьте_сюда_вывод_openssl
   ```

   Закройте доступ к файлу — в нём лежит ключ шифрования:
   ```bash
   ...
   - **`GENERIC_TIMEZONE` и `TZ`** — без них Schedule Trigger работает по UTC, и «каждый день в 9:00» окажется в 12:00 по Москве.
...
Каталог `/opt/n8n-data` нужен и при PostgreSQL — там лежит ключ шифрования.
...
- `/opt/n8n.env` — переменные окружения, включая `N8N_ENCRYPTION_KEY`.
...
Бэкап базы без `N8N_ENCRYPTION_KEY` бесполезен — учётные данные из него не расшифровать. Храните ключ отдельно от дампов.
