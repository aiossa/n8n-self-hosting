...
![Предупреждение Google Safe Browsing на странице входа n8n](images/google-safe-browsing.png)

Это не ошибка установки. Google Safe Browsing иногда помечает страницы входа свежих self-hosted сервисов как фишинговые: робот видит на новом домене форму логина и срабатывает по ошибке. Сертификат и сервер при этом в порядке.

Чтобы снять предупреждение:

1. Добавьте домен в [Google Search Console](https://search.google.com/search-console) и подтвердите владение — проще всего TXT-записью в DNS, там же, где создавали A-запись.
2. Откройте раздел **Безопасность и ручные меры → Проблемы безопасности** и нажмите **Запросить проверку**.
3. В комментарии напишите, что это личный экземпляр n8n — открытого инструмента автоматизации, а страница входа предназначена только для вас.

Проверка обычно занимает от суток до трёх. Пока она идёт, открыть n8n можно через ссылку «Подробности» → «Это небезопасная страница» внизу красного экрана.
